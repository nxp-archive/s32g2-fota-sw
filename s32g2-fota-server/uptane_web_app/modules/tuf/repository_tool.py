...
class Repository(object):
  ...
  def get_roledb_version(self,role):

    root_info = tuf.roledb.get_roleinfo('root', self.repository_name)
    targets_info = tuf.roledb.get_roleinfo('targets', self.repository_name)
    snapshot_info = tuf.roledb.get_roleinfo('snapshot', self.repository_name)
    timestamp_info = tuf.roledb.get_roleinfo('timestamp', self.repository_name)
    
    if role == 'root':
      return root_info['version']
    if role == 'targets':
      return targets_info['version']
    if role == 'snapshot':
      return snapshot_info['version']
    if role == 'timestamp':
      return timestamp_info['version']
...
class Targets(Metadata):
  ...
  """
    target_custom = None
    target_custom = tuf.roledb.get_roleinfo(self.rolename, self.repository_name)['path']

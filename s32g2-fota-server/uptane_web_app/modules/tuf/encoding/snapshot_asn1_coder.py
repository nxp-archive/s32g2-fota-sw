...
def get_asn_signed(pydict_signed):
  ...
  for filename in sorted_filenames:
    ...
    if filename == 'targets.' + tuf.conf.METADATA_FORMAT:
      ...
      fileinfo = SnapshotMetadataFile()
      fileinfo['filename'] = str(filename)
      ...
      if 'length' in pydict_fileinfo or 'hashes' in pydict_fileinfo:
        ...
        raise tuf.Error('ASN1 Conversion failure for Snapshot role: given '
            ...
            '(filename: ' +repr(str(filename)) + '), found either hashes or length, '
            'which are not expected in Snapshot for a Targets role file.')
      '''
      fileinfo['length'] = pydict_fileinfo['length']
      hashes = Hashes().subtype(
          implicitTag=tag.Tag(tag.tagClassContext,tag.tagFormatSimple, 4))
      number_of_hashes = 0

      #snapshotMetadataFiles[number_of_target_role_files] = number_of_target_role_files
      #number_of_target_role_files += 1
      #snapshotMetadataFiles['snapshotMetadataFiles'] = fileinfo
      ...
      for hashtype in sorted_hashtypes:
        ...
        hash['digest'] = OctetString(hexValue=hashval).subtype(implicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 1))
        hashes[number_of_hashes] = hash
        number_of_hashes += 1

      fileinfo['hashes'] = hashes
      fileinfo['numberOfHashes'] = number_of_hashes

      snapshotMetadataFiles[number_of_target_role_files] = fileinfo
      number_of_target_role_files += 1
    else:
      pass

  # Loop complete, all fileinfo (root, targets, any delegated targets)
  # loaded into snapshotMetadataFiles and root_fileinfo.
  ''' 
  if len(snapshotMetadataFiles) < 1:
  ...
  snapshot_metadata['numberOfSnapshotMetadataFiles'] = number_of_target_role_files
  snapshot_metadata['snapshotMetadataFiles'] = snapshotMetadataFiles
...
def get_json_signed(asn_metadata):
  ...
  asn_target_fileinfos = snapshot_metadata['snapshotMetadataFiles']

...
class Director:
  ...
  def register_ecu_serial(self, ecu_serial, ecu_key, vin, is_primary=False):
    ...
    inventory.register_ecu(is_primary, vin, ecu_serial, ecu_key, overwrite=False)
    '''
    if(SQL_USED):
      sql_db.check_vin_registered(vin)
      sql_db.register_ecu(is_primary, vin, ecu_serial, ecu_key, overwrite=False)
    '''
  ...
  def check_ecu_status(self,ecu_state):
      #TODO:
      if ecu_state == "kunregistered":
            print("the ECU is a unregistered, register this ECU")
      else:
          pass
  ...
  def validate_ecu_manifest(self, ecu_serial, signed_ecu_manifest):
    ...
    # If it doesn't match expect ations, error out here.
    ...
    #check_ecu_status(signed_ecu_manifest['signed']['state'])

    #if ecu_serial not in inventory.ecu_public_keys:
    '''
    if(SQL_USED):
      if ecu_serial not in sql_db.ecu_public_keys:
        log.info(
    ...
    if(SQL_USED):
      ecu_public_key = sql_db.ecu_public_keys[ecu_serial]
    '''
    '''
  ...
  def register_vehicle_manifest(
    ...
    print("signed_vehicle_manifest"+str(signed_vehicle_manifest))

    #[nick TODO:add the sqlDB]
    ...
    '''
    if(SQL_USED):
      sql_db.save_vehicle_manifest(vin, signed_vehicle_manifest)
    #[nick TODO:add the sqlDB]
    #sql_db.save_vehicle_manifest(vin, signed_vehicle_manifest)
    '''
    print("test4")
    log.info(GREEN + ' Received a Vehicle Manifest from Primary ECU ' +
        ...
        ENDCOLORS)
    # TODO: Note that the aove hasn't cbhecked that the signature was from
    ...
    # TODO:Note that the the DB instand of signed_vehicle_manifest 
    ...
    for ecu_serial in all_ecu_manifests:
      ...
      for manifest in ecu_manifests:
        try:
          ...
          # caught below.
          print('vin:'+vin+'   ecu_serial:'+ecu_serial+'    manifest:')
          print(manifest)
    ...
    print("register_vehicle_manifest DONE")          
  ...
  def validate_primary_certification_in_vehicle_manifest(
    ...
    '''
    if(SQL_USED):
      ecu_public_key = sql_db.ecu_public_keys[primary_ecu_serial]
    '''
  ...
  def register_ecu_manifest(self, vin, ecu_serial, signed_ecu_manifest):
    ...
    print("validate_ecu_manifest success ecu_serial:"+ecu_serial)
    # Otherwise, we save it:
    inventory.save_ecu_manifest(vin, ecu_serial, signed_ecu_manifest)
    print("save_ecu_manifest success")
    '''
    if(SQL_USED):
      sql_db.save_ecu_manifest(vin, ecu_serial, signed_ecu_manifest)
    '''
  ...
  def add_new_vehicle(self, vin, primary_ecu_serial=None):
    ...
    #sql_db.register_vehicle(vin, primary_ecu_serial=primary_ecu_serial)
  ...
  def add_target_for_ecu(self, vin, ecu_serial, target_filepath, custom=None):
    """
    ...
    add the custom parameter which is the additional data for target metdata
    """
    uptane.formats.VIN_SCHEMA.check_match(vin)
    uptane.formats.ECU_SERIAL_SCHEMA.check_match(ecu_serial)

...
def set_asn_targets(json_signed, targetsMetadata):
  ...
  for filename in sorted_filenames:
    ...
    target['version'] = json_signed['targets'][filename]['version'] 
    ...
    for hash_function in sorted_hash_functions:
      ...
      digest = OctetString(hexValue=hash_value).subtype(implicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 1))
      hash['digest'] = digest
    ...
    if 'custom' in filemeta:
      ...
      for customkey in sorted_customkeys:
        ...
        if customkey == 'dependency':
          custom['dependency'] = filemeta['custom'][customkey]
        elif customkey == 'isCompressed':
          custom['isCompressed'] = filemeta['custom'][customkey]
        elif customkey == 'installMethod':
          custom['installMethod'] = filemeta['custom'][customkey]
        elif customkey == 'imageFormat':
          custom['imageFormat'] = filemeta['custom'][customkey]
        elif customkey == 'hardwareVersion':
          custom['hardwareVersion'] = filemeta['custom'][customkey]
        elif customkey == 'ecuIdentifier':

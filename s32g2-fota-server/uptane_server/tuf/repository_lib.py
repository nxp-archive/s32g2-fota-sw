...
def _generate_and_write_metadata(rolename, metadata_filename, write_partial,
                                 ...
                                 consistent_snapshot=True, filenames=None,
  ...
  # a delegated role.
    ...
    metadata = generate_targets_metadata(targets_directory,
                                         ...
                                         consistent_snapshot)
    print("printf generate_targets_metadata")
    print(metadata)
...
def get_metadata_fileinfo(filename, file_version=None, custom=None):
  """
  <Purpose>
  ...
  return tuf.formats.make_fileinfo(filesize, filehashes, version=file_version, custom=custom)
...
def generate_targets_metadata(targets_directory, target_files, version,
                              ...
                              write_consistent_targets=True,update_version=None):
  """
  <Purpose>
...
CUSTOM_SCHEMA
  ...
  for target, custom in six.iteritems(target_files):
    ...
    relative_targetpath = target.strip('/')
    ...
    if len(custom):
      custom_data = custom
      update_version = custom['dependency']
      print("update_version:" + str(custom))

      filedict[relative_targetpath] = get_metadata_fileinfo(target_path,update_version, custom_data) 
     
      
    else:
      #TODO add image version number
      # version_number = {"version_num":2}
      pass
      #filedict[relative_targetpath] = \
        #get_metadata_fileinfo(target_path, version_number)
  ...
  if delegations is not None:
    return tuf.formats.build_dict_conforming_to_schema(
        ...
        expires=expiration_date,
        targets=filedict,
        write_consistent_targets=write_consistent_targets,
...
def generate_snapshot_metadata(metadata_directory, version, expiration_date,
  ...
  targets_path = os.path.join(
      metadata_directory, targets_filename + METADATA_EXTENSION)
  length, hashes = tuf.util.get_file_details(targets_path)
  ...
  #fileinfodict[TARGETS_FILENAME] = get_metadata_versioninfo(targets_filename, repository_name)
  targets_version = get_metadata_versioninfo('targets', repository_name)
  fileinfodict[TARGETS_FILENAME] = tuf.formats.make_fileinfo(length, hashes, version=targets_version['version'])

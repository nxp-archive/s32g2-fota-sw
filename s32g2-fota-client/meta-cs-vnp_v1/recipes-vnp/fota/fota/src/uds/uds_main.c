...
#include "uds/doip_client.h"
#include "uds/uds_file_linux.h"
...
int get_ota_state(struct udsc *uds, uint8_t *errcode, tOTAState *state, uint8_t *progress)
{
    ...
    g_routine_opts[UDS_TRANSFER_ID_LEN] = 0;
...
int uds_client_file_op(const char *server_ip, char file_op, int file_num, char file_path[][128], udsc_cb callback)
{
	int ret;
	struct uds_tp *tp;
	struct udsc *uds;
        ...
        case 'r':
            {
                ...
                for (i = 0; i < file_num; i++) {
                    ...
                    printf("file info: name=%s, size=%d, crc=0x%x\n",
                        udsfile->name, udsfile->uncompressed_size, udsfile->crc);
                    
                    ret = file_op == 'a' ? udsc_add_file(uds, udsfile, callback) : udsc_replace_file(uds, udsfile, callback);
                    if (ret != 0) {
                        printf("%s file .ret = %d\n", file_op == 'a' ? "Add" : "Replace", ret);
...
int uds_client_cmd_set(const char *server_ip, char op)
{
	int ret;
	struct uds_tp *tp;
	struct udsc *uds;
...
int uds_client_get_state(const char *server_ip, uint8_t *err, uint8_t *state, uint8_t *progress)
{
	int ret;
	struct uds_tp *tp;
	struct udsc *uds;
...

    {
	    ...
	    ret = get_ota_state(uds, err, &ota_state, progress);
		if (ret) {
			fprintf(stderr, "get_ota_state.ret = %d\n", ret);
		}

        *state = (uint8_t)ota_state;
...
int uds_main(int argc, char *argv[])
{
	...
	while ((opt = getopt(argc, argv, "c:a:r:d:iefs")) != -1) {
		switch (opt) {
		case 'c':
			server_ip = optarg;
			break;
	...
	case 'r':
		{
			...
			for (i = 0; i < file_num; i++) {
				...
				ret = file_action == 'a' ? udsc_add_file(uds, udsfile) : udsc_replace_file(uds, udsfile);
	...
	case 'i':
		ret = set_ota_state(uds, OTA_INSTALL_FW);
		if (ret) {
			fprintf(stderr, "set ota state to INSTALL fail\n");
			break;
		}
		break;
	case 'e':
		ret = set_ota_state(uds, OTA_ACTIVATE);
		if (ret) {
			fprintf(stderr, "set ota state to ACTIVATE fail\n");
			break;
		}
		break;
	case 'f':
		ret = set_ota_state(uds, OTA_FINISH);
		if (ret) {
			fprintf(stderr, "set ota state to FINISH fail\n");
		}
		break;
	case 's':
		{
